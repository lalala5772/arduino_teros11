...
// 간단한 선형 매핑 함수 (7.34~68 → 0~70)
float mapMoisture(float original) {
  float mapped = ((original - 7.34) / (68.0 - 7.34)) * 70.0;
...
void loop() {
  ...
  // Average readings for each parameter (원본과 동일)
  soilMoistMean   /= (NUMSAMPLES*2000);
  degCMean        /= NUMSAMPLES;

  // 매핑 적용 (7.34~68 → 0~70)
  float mappedValue = mapMoisture(soilMoistMean);
  ...
  // 상태 판정 (0~70 기준)
  char* status;
  if(mappedValue < 21){    // 30% of 70
    status = "low";
    Serial.println(status);
  }
  else if(mappedValue < 49){  // 70% of 70
    status = "good";
    Serial.println(status);
  }
  else {
