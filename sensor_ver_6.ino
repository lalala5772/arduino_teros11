...
// 실제 측정값 기준 (공기와 물에서의 값)
#define AIR_READING 7.34    // 공기 중에서 나오는 값
#define WATER_READING 60.0  // 물 속에서 나오는 값 (추정)
...
void setup() {
  ...
  Serial.println("Output: 0-70");
  Serial.println("=========================");
}

// 센서값을 0-70으로 매핑
float mapToScale(float reading) {
  // 7.34-60 범위를 0-70으로 매핑
  float mapped = (reading - AIR_READING) / (WATER_READING - AIR_READING) * 70.0;
  
  // 범위 제한
  if (mapped < 0) mapped = 0;
  if (mapped > 70) mapped = 70;
...
void loop() {
  ...
  // 7.34-60 범위를 0-70으로 매핑
  float scaledValue = mapToScale(soilMoistMean);
